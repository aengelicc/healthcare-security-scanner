...
"insecure-deserialization-pickle": {
    ...
    "steps": [
        ...
        "If pickle is absolutely necessary, validate data integrity with HMAC before deserializing",
...
```
Semgrep prefixes rule IDs with the config file name when loading from a
local YAML (e.g. 'healthcare_rules.hardcoded-credentials-all-languages').
We try the full ID first, then fall back to the bare rule name after the
last dot so that lookups succeed regardless of how the config was invoked.
"""
if rule_id in REMEDIATION_TEMPLATES:
    return REMEDIATION_TEMPLATES[rule_id]

# Strip config-file prefix (everything up to and including the last '.')
short_id = rule_id.rsplit(".", 1)[-1]
return REMEDIATION_TEMPLATES.get(short_id, _DEFAULT_REMEDIATION)
```

def generate_remediation_section(findings: list) -> list:
“”“Generate a list of remediation dicts for all findings.

```
Each entry merges finding metadata (rule_id, file, line, severity) with
the matched remediation template, ready for report consumption.
"""
sections = []
